...
AI tutoring web app on Next.js 16 App Router + React 19 + Tailwind v4: a Mastra agent served to a CopilotKit chat over AG-UI, behind Better Auth email/password sign-in, over a Drizzle/SQLite persistence layer, with a Bearer-token REST API for the same todos and a Vitest + Playwright test harness.
...
- `lib/auth.ts` is the app instance (explicitly `server-only`, `bearer()` for the REST API, `nextCookies()` last); `lib/auth-cli.ts` exists only because the Better Auth CLI refuses to load a module graph containing `server-only`.
...
## REST API — `app/api/todos/`, `lib/todo-api.ts`, `lib/api-session.ts`

- `GET /api/todos?q=`, `POST /api/todos` and `PATCH /api/todos/[id]` serve CLIs and services, and their request, response and error shapes are the zod schemas in `lib/todo-api.ts`, which must import nothing but zod so a client can load it.
- The token is the signed session token Better Auth returns in the `set-auth-token` header on sign-in, so signing out or revoking that session revokes it and no extra table exists.
...
- The six `.test.ts` files in `tests/unit` select the node environment, while `todo-tool-calls.test.tsx` uses jsdom; the db, auth, tutor and todos-api tests point at a temp file, so they never touch `data/app.db`.
...
## Tooling — `biome.json`, `.gitattributes`
